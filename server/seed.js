...
async function main() {
    ...
    try {
        ...
        const productsCollection = client.db("burger_app").collection("products");
        const categoriesCollection = client.db("burger_app").collection("categories");

        let categories = ['veg', 'non-veg'].map((category) => { return { name: category } });
